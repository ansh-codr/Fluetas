        ...
        {[
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold shrink-0 transition-all border cursor-pointer ${
              filter === f.id
                ? 'bg-[#2E6DA4]/10 text-[#2E6DA4] border-[#2E6DA4]/30 font-bold'
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
              <div className="flex items-start gap-3.5">
                <div className="w-12 h-12 rounded-xl bg-[#2E6DA4]/10 border border-[#2E6DA4]/20 flex items-center justify-center text-[#2E6DA4] text-xl shrink-0">
                  <FileText size={22} />
                </div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="px-2 py-0.5 rounded text-[0.62rem] font-bold uppercase bg-[#F2F4EE] text-[#586151]">
                      {doc.documentType}
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-2 self-end sm:self-auto">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-[#F2F4EE] border border-[rgba(18,22,15,0.10)] text-[#12160F] hover:bg-[#E8ECE2] text-xs font-semibold flex items-center gap-1.5 cursor-pointer"
                >
                  <Eye size={13} /> View
                </button>
                <button
                  onClick={() => alert(`Downloading verified copy of ${doc.reportName}`)}
                  className="px-3 py-1.5 rounded-lg bg-[#2E7D32]/10 text-[#2E7D32] border border-[#2E7D32]/20 hover:bg-[#2E7D32]/20 text-xs font-semibold flex items-center gap-1.5 cursor-pointer"
                >
            ...
            <div className="bg-[#F2F4EE] p-3.5 rounded-xl border border-[rgba(18,22,15,0.08)]">
              <span className="text-[0.65rem] font-bold text-[#586151] uppercase tracking-wider block mb-2.5">
                Clinical Workflow Status: <strong className="text-[#2E7D32]">{doc.status}</strong>
              </span>
                  ...
                  return (
                    <div key={step} className="flex flex-col items-center gap-1">
                      <div className={`h-1.5 w-full rounded-full ${
                        isCompleted ? 'bg-[#2E7D32]' : 'bg-[rgba(18,22,15,0.15)]'
                      }`} />
                      <span className={`text-[0.62rem] font-semibold truncate w-full ${
                        isCompleted ? 'text-[#2E7D32]' : 'text-[#8A9482]'
                      }`}>
                        {step}
                      </span>
                    </div>
            ...
            {doc.testParameters && (
              <div className="bg-[#F2F4EE] rounded-xl border border-[rgba(18,22,15,0.08)] overflow-hidden text-xs">
                <div className="p-2.5 bg-white border-b border-[rgba(18,22,15,0.08)] flex items-center justify-between">
                  <span className="font-bold text-[#12160F] flex items-center gap-1.5">
                    <FileSpreadsheet size={14} className="text-[#2E7D32]" />
                    Extracted Diagnostic Telemetry
                  </span>
                  <span className="text-[0.68rem] text-[#586151]">Verified by Clinical Parser</span>
                </div>

                <div className="p-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                  {doc.testParameters.map((param, i) => (
                    <div key={i} className="p-2.5 rounded-lg bg-white border border-[rgba(18,22,15,0.08)] flex flex-col justify-between">
                      <span className="text-[0.68rem] text-[#586151] truncate">{param.test}</span>
                      <div className="flex items-center justify-between mt-1">
                        <strong className="text-[#12160F] text-sm">{param.value}</strong>
                        <span className="text-[0.62rem] font-bold px-1.5 py-0.5 rounded" style={{ backgroundColor: `${param.color}15`, color: param.color }}>
                          {param.status}
                        </span>
                      </div>
                      <span className="text-[0.62rem] text-[#8A9482] mt-1">Ref: {param.refRange}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {doc.doctorReview && (
              <div className="bg-[#2E7D32]/10 border border-[#2E7D32]/25 p-3 rounded-xl text-xs text-[#12160F] flex flex-col gap-1">
                ...
                <p className="m-0 text-[#586151]">{doc.doctorReview.findings}</p>
                <p className="m-0 font-semibold text-[#2E7D32] mt-0.5">Protocol: {doc.doctorReview.recommendation}</p>
              </div>
            )}
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white border border-[rgba(18,22,15,0.12)] rounded-2xl p-6 max-w-md w-full shadow-2xl relative animate-slide-up">
            ...
            <form onSubmit={handleUpload} className="flex flex-col gap-3.5 text-xs">
              <div>
                <label className="block text-[#586151] font-semibold mb-1">Document Title</label>
                <input
                  required
                  placeholder="e.g. Thyroid & Lipid Profile"
                  value={reportName}
                  onChange={e => setReportName(e.target.value)}
                  className="w-full bg-white border border-[rgba(18,22,15,0.15)] rounded-lg p-2.5 text-[#12160F] focus:outline-none focus:border-[#2E7D32]"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#586151] font-semibold mb-1">Type</label>
                  <select
                    value={docType}
                ...
                <div>
                  <label className="block text-[#586151] font-semibold mb-1">Diagnostic Center</label>
                  <input
                    value={facility}
              ...
              {/* Upload Dropzone */}
              <div className="border-2 border-dashed border-[rgba(18,22,15,0.15)] hover:border-[#2E7D32] p-6 rounded-xl text-center bg-[#F2F4EE] cursor-pointer transition-colors">
                <Upload size={24} className="text-[#2E7D32] mx-auto mb-2" />
                <p className="font-bold text-[#12160F] m-0">Click to upload or drag PDF/DICOM file</p>
                <p className="text-[0.65rem] text-[#586151] m-0 mt-1">Encrypted on Firebase Storage · Up to 50MB</p>
              </div>
